...
fn init(alloc: Allocator, dbg: *Debugger) !*Self {
    ...
    const window = try glfw.Window.create(width, height, "Panacea", null);
...
fn drawMenuBar(self: *Self) ?State.View {
    ...
    if (zui.beginMainMenuBar()) {
        ...
        if (zui.beginMenu("File", true)) {
            ...
            if (zui.menuItem("panacea", .{ .shortcut = "q" })) {
                self.state.quit();
            }
        }
        ...
        if (zui.beginMenu("About", true)) {
            ...
            zui.text("Thank you for using panacea! This software is in early alpha, so there will be bugs.", .{});
            zui.text("", .{});
